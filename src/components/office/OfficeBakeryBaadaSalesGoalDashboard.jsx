...
// Constants
const STORAGE_KEY = "bakery_baada_yearly_sales_goal";
const MIN_GOAL = 0;
...
const OfficeBakeryBaadaSalesGoalDashboard = () => {
  ...
  const actualSalesData = useMemo(() => {
    ...
    officeBook.forEach((entry) => {
      ...
      // Calculate daily total from officeIn entries (Bakery Baada only)
      const dailyTotal =
        entry?.officeIn
          ?.filter((item) => {
            const category = (
  ...
  return (
    ...
    >
      <Typography variant="h5" fontWeight={700} textAlign="center" mb={1}>
        🏢 Office Bakery Baada Sales Goal Dashboard
      </Typography>
...
export default OfficeBakeryBaadaSalesGoalDashboard;
